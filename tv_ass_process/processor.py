...
class Processor:
    ...
    # 修改 process_and_save 方法：
    def process_and_save(self, input_path: Path, output_path: Path) -> None:
        logger.info(f"Starting processing {input_path}")
        doc = Subtitle.load(input_path)
        self.process_subtitle(doc)
        doc.save(output_path)  # 直接保存为SRT，使用固定配置
    ...
    def process_subtitle(self, doc: Subtitle) -> None:
        ...
        for event in doc.events:
            event.text = clean_up_text(event.text)  # 关键行
